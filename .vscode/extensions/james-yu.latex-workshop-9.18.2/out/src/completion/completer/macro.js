...
exports.isTriggerSuggestNeeded = exports.macro = exports.provider = void 0;
...
const logger = lw_1.lw.log('Intelli', 'Macro');
...
Object.entries(JSON.parse(fs.readFileSync(`${lw_1.lw.extensionRoot}/data/unimathsymbols.json`).toString()))
...
function initialize() {
    ...
    const maths = excludeDefault ? {} : JSON.parse(fs.readFileSync(`${lw_1.lw.extensionRoot}/data/packages/tex.json`, { encoding: 'utf8' })).macros;
...
function entryCmdToCompletion(itemKey, item) {
    item.macro = item.macro || itemKey;
    const backslash = item.macro.startsWith(' ') ? '' : '\\';
    const suggestion = new completerutils_1.CmdEnvSuggestion(`${backslash}${item.macro}`, item.package || 'latex', item.keyvals && typeof (item.keyvals) !== 'number' ? item.keyvals : [], item.keyvalpos === undefined ? -1 : item.keyvalpos, (0, completerutils_1.splitSignatureString)(itemKey), vscode.CompletionItemKind.Function, item.option);
    if (item.snippet) {
...
function setPackageCmds(packageName, cmds) {
    const macros = [];
