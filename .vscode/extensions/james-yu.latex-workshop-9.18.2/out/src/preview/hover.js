...
class HoverProvider {
    async provideHover(document, position, ctoken) {
        lw_1.lw.preview.math.refreshMathColor();
        ...
        if (hov) {
            ...
            if (tex) {
                const hover = await lw_1.lw.preview.math.onTeX(document, tex, await (0, newcommandfinder_1.findMacros)(ctoken));
                return hover;
            }
        ...
        if (hovReference && refData) {
            const hover = await lw_1.lw.preview.math.onRef(document, position, refData, ctoken);
            return hover;
        }
