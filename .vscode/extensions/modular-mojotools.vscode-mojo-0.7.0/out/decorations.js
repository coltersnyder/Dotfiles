...
exports.MojoDecoratorContext = void 0;
 ...
 */
class MojoDecoratorContext extends disposableContext_1.DisposableContext {
    constructor() {
        super();
        ...
        this.pushSubscription(vscode.workspace.onDidOpenTextDocument(event => { this.decorateDocument(event); }));
        this.pushSubscription(vscode.workspace.onDidChangeTextDocument(event => { this.decorateDocument(event.document); }));
        this.pushSubscription(vscode.workspace.onDidOpenNotebookDocument(event => { this.decorateNotebookDocument(event); }));
        this.pushSubscription(vscode.workspace.onDidChangeNotebookDocument(event => { this.decorateNotebookDocument(event.notebook); }));
        this.pushSubscription(vscode.window.onDidChangeVisibleTextEditors(editors => { editors.forEach(editor => this.decorate(editor)); }));
        this.pushSubscription(vscode.window.onDidChangeVisibleNotebookEditors(editors => {
            editors.forEach(editor => this.decorateNotebookDocument(editor.notebook));
        }));
    ...
    decorateNotebookDocument(notebook) {
...
exports.MojoDecoratorContext = MojoDecoratorContext;
