...
async function onTeX(document, tex, macros) {
    ...
    const typesetArg = macros + mathpreviewutils_1.MathPreviewUtils.stripTeX(s, macros);
...
async function onRef(document, position, refData, token, ctoken) {
    ...
    if (configuration.get('hover.ref.enabled')) {
        ...
        if (tex) {
            return hoverpreviewonref_1.HoverPreviewOnRefProvider.provideHoverPreviewOnRef(tex, await (0, newcommandfinder_1.findMacros)(ctoken), refData, color);
        }
    }
...
async function generateSVG(tex, macros) {
    ...
    const xml = await typeset(macros + mathpreviewutils_1.MathPreviewUtils.stripTeX(s, macros), { scale, color });
...
async function renderSvgOnRef(tex, refData, ctoken) {
    return hoverpreviewonref_1.HoverPreviewOnRefProvider.renderSvgOnRef(tex, await (0, newcommandfinder_1.findMacros)(ctoken), refData, color);
}
